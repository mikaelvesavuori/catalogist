...
describe('Failure cases', () => {
    describe('Validation errors', () => {
        test('It should throw a ValidationError if missing "spec" field', async () => {
            expect(async () => await createRecord(repo, createRecordMissingSpecField)).rejects.toThrowError(ValidationError);
        });
        describe('Length errors', () => {
            test('It should throw a ValidationError if there are more than 10 items in the "api" field', async () => {
                const recordTooManyApiItems = JSON.parse(JSON.stringify(createRecordValidBasic));
                recordTooManyApiItems['api'] = tooLongArray;
                expect(async () => await createRecord(repo, recordTooManyApiItems)).rejects.toThrowError(ValidationError);
            });
            test('It should throw a ValidationError if there are more than 10 items in the "slo" field', async () => {
                const recordTooManySloItems = JSON.parse(JSON.stringify(createRecordValidBasic));
                recordTooManySloItems['slo'] = tooLongArray;
                expect(async () => await createRecord(repo, recordTooManySloItems)).rejects.toThrowError(ValidationError);
            });
            test('It should throw a ValidationError if there are more than 10 items in the "links" field', async () => {
                const recordTooManyLinkItems = JSON.parse(JSON.stringify(createRecordValidBasic));
                recordTooManyLinkItems['links'] = tooLongArray;
                expect(async () => await createRecord(repo, recordTooManyLinkItems)).rejects.toThrowError(ValidationError);
            });
            test('It should throw a ValidationError if there are more than 10 items in the "tags" field', async () => {
                const recordTooManyTagItems = JSON.parse(JSON.stringify(createRecordValidBasic));
                recordTooManyTagItems.spec.tags = tooLongArray;
                expect(async () => await createRecord(repo, recordTooManyTagItems)).rejects.toThrowError(ValidationError);
            });
            test('It should throw a ValidationError if there are more than 50 items in the "relations" field', async () => {
                const recordTooManyRelationItems = JSON.parse(JSON.stringify(createRecordValidBasic));
                ...
                expect(async () => await createRecord(repo, recordTooManyRelationItems)).rejects.toThrowError(ValidationError);
            });
    ...
    test('It should throw a MissingSpecKeysError if missing "name" field', async () => {
        expect(async () => await createRecord(repo, createRecordMissingServiceNameField)).rejects.toThrowError(MissingSpecKeysError);
    });
...
describe('Success cases', () => {
    ...
    test('It should create a record and keep a regular free-form description intact', async () => {
        const input = JSON.parse(JSON.stringify(createRecordValidFull));
        const description = 'Markus Åhström and Wikipedia writes "asdf!#)/€" An application programming interface (@API) is a way for two or more computer programs to communicate with each other. It is a type of software interface, offering a service to other pieces of software.[1] A document or standard that describes how to build or use such a connection or interface is called an API specification. A computer system that meets this standard is said to implement or expose an API. The term API may refer either to the specification or to the implementation. In contrast to a user interface, which connects a computer to a person, an application programming interface connects computers or pieces of software to each other. It is not intended to be used directly by a person (the end user) other than a computer programmer who is incorporating it into the software. An API is often made up of different parts which act as tools or services that are available to the programmer.';
